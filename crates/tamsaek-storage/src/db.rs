...
impl Database {
    pub fn open(config: DatabaseConfig) -> StorageResult<Self> {
        ...
        // so it applies to this connection
        unsafe {
    ...
    pub fn initialize_schema(&self) -> StorageResult<()> {
    ...
    where
        F: FnMut(&rusqlite::Row<'_>) -> StorageResult<T>,
    {
        let conn = self.conn.lock();
    ...
    pub fn transaction<T, F>(&self, f: F) -> StorageResult<T>
    where
    ...
    /// Get a reference to the connection for batch operations
