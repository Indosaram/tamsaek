//! Nom-based query parser for the Query DSL.
//!
//! This parser handles:
...
//! - Field filters (from:, ext:, date:, size:, etc.)

use super::dsl::*;
use crate::TamsaekError;
...
/// Query parser that converts text queries into structured Query objects.
...
impl QueryParser {
    ...
    ///
    /// ```
    /// use tamsaek_core::query::QueryParser;
    ///
    /// let query = QueryParser::parse("rust async").unwrap();
    /// // Parsed as: And([Term("rust"), Term("async")])
    /// ```
    pub fn parse(input: &str) -> Result<Query, TamsaekError> {
        let input = input.trim();
        if input.is_empty() {
            return Err(TamsaekError::Parse("Empty query".to_string()));
        }

        match all_consuming(preceded(multispace0, parse_query))(input) {
            Ok((_, query)) => Ok(query),
            Err(nom::Err::Error(e)) | Err(nom::Err::Failure(e)) => {
                Err(TamsaekError::Parse(format!(
            ...
            Err(nom::Err::Incomplete(_)) => {
                Err(TamsaekError::Parse("Incomplete query".to_string()))
            }
        }
    }
}
...
fn parse_filter(input: &str) -> ParseResult<'_, Query> {
    ...
    let filter = match key.to_lowercase().as_str() {
        ...
        "date" | "modified" | "created" | "indexed" => {
            let field = DateField::parse(key).unwrap_or(DateField::Any);
...
fn parse_date_value(value: &str) -> Option<DateValue> {
    ...
    if let Some(preset) = DatePreset::parse(value) {
